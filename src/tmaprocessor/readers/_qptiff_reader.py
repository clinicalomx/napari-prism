...
from napari_spatialdata._reader import reader_function as native_napari_spatialdata_reader
...
class QptiffLoader(ABC):
    ...
    def load_page_series(self, qptiff_path, channel_int=None, with_zarr=False):
        if with_zarr:
            ...
            self.image = da.from_zarr(z) # for z in data] # Can parse these as ImageModel2D's (multi)
    ...
    def generate_metadata(self):
        raise NotImplementedError("Calling abstract method from abstract class.")
    ...
    def to_spatialdata(self, downscale_factors=[2, 2, 2, 2]):
        ...
        transformations["um"] = Scale(
            ...
            axes=("x","y")).inverse() # converts pixels to um using qptiff provided metadata for scaling
        ...
        image = Image2DModel.parse(
            ...
            scale_factors=downscale_factors) # Passing scale factors with tuples -> MultiscaleSpatialImage
...
class PCFLoader(QptiffLoader):
    ...
    def generate_metadata(self):
        ...
        for i, page in enumerate(self.page_series):
            pandas_series = pd.read_xml(StringIO(page.description), parser="etree").stack()
        ...
        channel_exposure = pd.DataFrame(scan_profile["experimentDescription"]["channels"])
        unused_items = pd.DataFrame(scan_profile["experimentDescription"]["unusedItems"])
        ...
        self.set_channel_map({v:k for k,v in data_df.Biomarker.to_dict().items()})
...
def qptiff_reader_function(
    ...
    napari_spatialdata_reader(target_path)
    
    # Then launch TMA plugins
    return [(None,)]

def napari_spatialdata_reader(
    path: str | Path):
...
def napari_get_reader(path: str):
    ...
    if path.endswith(".qptiff"):
        ...
        return napari_spatialdata_reader
