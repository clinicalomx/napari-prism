...
def filter_by_obs_count(
    ...
    """
    Filters cells by a min and/or max value in a defined obs_col. 
    If layer is None, does this on .X.
    """
    MIN_DEFAULT = 10
    MAX_DEFAULT = 1000000

    if not isinstance(adata.obs[obs_col].dtype, pd.CategoricalDtype) or \
        pd.api.types.is_numeric_dtype(adata.obs[obs_col]):
...
def filter_by_obs_value(
    ...
    if isinstance(adata.obs[obs_col].dtype, pd.CategoricalDtype) or not \
        pd.api.types.is_numeric_dtype(adata.obs[obs_col]):
...
def filter_by_obs_quantile(
    ...
    if min_quantile is None and max_quantile is None:
        ...
        print(f"Excluding cells with {obs_col} below the {MIN_DEFAULT} percentile")
        min_quantile = MIN_DEFAULT
        
        print(f"Excluding cells with {obs_col} above the {MAX_DEFAULT} percentile")
...
class AnnDataProcessor():
    ...
    """ Processes a singular AnnData object, assumed to be derived from one 
        medium (image/slide/etc).
    """
    def __init__(
        self, 
        adata: AnnData | None
    ...
    def _raise_error_if_applied(self, partial_function):
        if partial_function.func in self.applied_funcs:
            raise RuntimeError("Function already applied. Skipping operation...")
    ...
    def _cache_applied_function(self, partial_function, tandem_apply):
        ...
        self.applied_funcs.append({function_name: function_kwargs}) # {function : {'a': val, 'b': val}}}
    ...
    def arcsinh_transform(self, cofactor=150):
        ...
        print(f"Applying arcsinh transformation with cofactor: {cofactor}", flush=True)
    ...
    def percentile_transform(self, percentile=95):
        ...
        self.apply_all(_percentile_transform, attr="X", percentile=percentile, axis=0) 
...
class AnnDataProcessorGPU(AnnDataProcessor):
    ...
    def check_if_GPU_version_installed(self):
        try:
            ...
            raise ImportError(
                "rapids_singlecell not installed. \
    ...
    def _check_if_rsc(self, partial_function):
        if partial_function.func.__module__.startswith("rapids_singlecell"):
            return True
        else:
            return False
...
def split_by_obs(
    ...
    for selection in selections:
        adata_list.append(adata[adata.obs[obs_var] == selection].copy()) # TODO; try catch selections
    print(f"Got {adata}")
    print(f"Spliting by {obs_var}")
    print(f"" if selections is None else f"Got selections: {selections}")
    
    return adata_list, obs_var
    
class AnnDataCollection():
    """ Processes Anndata objects the same way and in parallel."""
    def __init__(self, adatas: Union[List[AnnData], AnnData], obs_var: str=None):
        self.adatas = adatas
        self.batch_key = obs_var
        self.merged = None
        self.applied_funcs = list() # Track applied functions;
    ...
    def _apply_parallel(self, partial_function, attr):
        ...
        adatas = Parallel(n_jobs=len(self.adatas), prefer="processes")(delayed(self._process)(adata, attr, partial_function, True) for adata in self.adatas)
        self.adatas = adatas
    ...
    def apply_all(
        ...
        tandem_apply: Optional[bool]=False, # If u can apply the same function twice in a row
        **function_kwargs):
    ...
    def harmonise_adatas(self):
        ...
        sc.external.pp.harmony_integrate(self.merged, key=self.batch_key, max_iter_harmony=40)
    ...
    def get_GPU_version(self):
        """ Copy func """
        adata_collection_gpu = AnnDataCollectionGPU(self.adatas, self.batch_key)
...
class AnnDataCollectionGPU(AnnDataCollection):
    ...
    def _apply_parallel(self, partial_function, attr):
        ...
        if self._check_if_rsc(partial_function):
            raise NotImplementedError("Parallel GPU implementations unsupported.")
