...
from ._widget_utils import get_layer_index_by_name, get_layer_names, \
    SingleScaleImageNapariWidget, MultiScaleImageNapariWidget, \
        create_static_selection_widget, get_selected_layer, \
        MultiScaleImageOperations, SingleScaleImageOperations
...
class UtilsNapariWidget(MultiScaleImageNapariWidget):
    ...
    def add_selected_image_channel(self):
        ...
        image_list = [
            image.sel(c=channel)[x].image for x in image
        ]
    ...
    # def add_singlescale_image(self):
    #     """ Add the single scale version of the image for compatbility with
    #         other napari plugins. """
    #     scale = self.get_selected_scale()
    #     image = self.model.get_image_by_scale(scale)
    ...
    def overwrite_save_selected_layer(self):
        ...
        def _save_annotated_shapes_layer(layer):
            ...
            annotations_inherited = layer.metadata["_columns_df"]["tma_label"].values
            ...
            gdf_recon = gpd.GeoDataFrame({
                "geometry": geoms,
                "tma_label": annotations_inherited
            })
        ...
        if isinstance(layer, napari.layers.shapes.shapes.Shapes) and \
            "_columns_df" in layer.metadata and \
            "sdata" in layer.metadata and \
            "name" in layer.metadata and \
            isinstance(layer.data, list):
...
#        self.update_model()
    ...
    def get_bbox_shape_layers(self, widget=None):
        ...
        if self.model is not None:
            for l in self.viewer.layers:
                if isinstance(l, napari.layers.shapes.shapes.Shapes):
                    if len(l.shape_type) == 1: # If the shapes layer contains only 1 shape;
                        if l.shape_type[0] == "rectangle":
                            rects = [None, l.name]
        return rects

    def get_bbox_shape_bounds(
        self, 
        bbox_layer: napari.layers.shapes.shapes.Shapes
    ...
    def create_parameter_widgets(self):
        ...
        self._sigma_slider = create_widget(
            ...
            annotation=float,
            options=dict(min=0, step=0.25, max=50),
            widget_type="SpinBox")
        ...
        self._expansion_slider = create_widget(
            ...
            annotation=int,
            options=dict(min=0, step=1, max=100),
            widget_type="SpinBox")
    ...
    def _generate(self): # Similar to nested _generate 
        ...
        self.model.mask_tma_cores(
            ...
            estimated_core_diameter_um=float(self._estimated_core_diameter_um_entry.value),
...
class TMADearrayerNapariWidget(SingleScaleImageNapariWidget):
    ...
    def get_line_shape_layers(self, widget=None):
        ...
        if self.model is not None:
            for l in self.viewer.layers:
                if isinstance(l, napari.layers.shapes.shapes.Shapes):
                    if l.shape_type[0] == "line":
                        lines = [None, l.name]
        return lines

    def get_line_mean_lengths(
        self, 
        line_layer: napari.layers.shapes.shapes.Shapes
    ) -> float:
    ...
    def _dearray(self):
        ...
        if self.model.image_name + "_poly" in self.model.sdata:
            masks_gdf = self.model.sdata[self.model.image_name + "_poly"]
        else:
            masks_gdf = None
        ...
        self.model.dearray_and_envelope_tma_cores(
            ...
            masks_gdf=masks_gdf) # Sets self.gff
...
#        grid_positions = self.model.grid_positions
        ...
        for i in range(grid.native.columnCount()):
            grid.native.horizontalHeader().setSectionResizeMode(i, grid.native.horizontalHeader().Stretch)
    ...
    def get_child_shape_layers(self, widget=None):
        ...
        if input_layer_name is None:
            ...
            return [
                l.name for l in self.viewer.layers if 
                    isinstance(l, napari.layers.shapes.shapes.Shapes)
                ]
    ...
    def show_annotations(self):
        ...
        if "_columns_df" in layer.metadata.keys():
            if "tma_label" in layer.metadata["_columns_df"]:
...
class TMASegmenterNapariWidget(MultiScaleImageNapariWidget):
    def __init__(
        ...
        **kwargs
        ...
        self.viewer.layers.selection.events.changed.connect(
            self.update_model)
        # self._shape_selection_widget = ComboBox(
        #     name="ShapeLayers",
        #     choices=self.get_shape_layers,
        #     label="Select shapes layer for tiling",
        #     value=self.get_shape_layers()[-1] # Last shape layer by default
        # )
        # self.set_working_shapes(self._shape_selection_widget)
        # self._shape_selection_widget.changed.connect(self.set_working_shapes)
        # self._shape_selection_widget.changed.connect(self.update_model)

        # self.extend(
        #     [
        #         self._shape_selection_widget,
        #     ]
        # )
...
#     def get_shape_layers(self, widget=None):
#         return [
#             l.name for l in self._viewer.layers if 
#                 isinstance(l, napari.layers.shapes.shapes.Shapes)
#             ]

#     def set_working_shapes(self, shapes_widget):
#         self.working_shapes_layer = get_selected_layer(self._viewer, 
#             shapes_widget)

#         self.working_shapes_gff = self.working_shapes_layer.metadata["gdf"]#pd.DataFrame(self.working_shapes_layer.metadata)
    ...
    def create_parameter_widgets(self):
        ...
        self._tiling_shape_layer_selection.changed.connect(
            self.reset_choices)
        ...
        self._nuclei_diameter_um_entry = create_widget(
            ...
            widget_type="LineEdit",
            options=dict(
                tooltip="If value is 0 or negative, will default to automated" \
                    " diameter estimation.")
            )
    ...
    def get_tiling_shape_layers(self, widget=None):
        return [
            l.name for l in self.viewer.layers if 
                isinstance(l, napari.layers.shapes.shapes.Shapes) and \
                "sdata" in l.metadata and \
                l.name in l.metadata["sdata"].shapes and \
                isinstance(l.metadata["sdata"].shapes[l.name], gpd.GeoDataFrame)
            ]
    ...
    def get_tiling_shape_layer_cols(self, widget=None):
        if self._tiling_shape_layer_selection.value is not None:
            layer = get_selected_layer(
                self.viewer, 
                self._tiling_shape_layer_selection.value)
            layer_gdf = layer.metadata["sdata"].shapes[layer.name]
            cat_cols = [
                x for x in layer_gdf.columns \
                    if isinstance(layer_gdf[x].dtype, pd.CategoricalDtype) or \
                        isinstance(layer_gdf[x].dtype, pd.StringDtype) or \
                        pd.api.types.is_string_dtype(layer_gdf[x])
            ]
            return cat_cols
        else:
            return []
    ...
    def _run_segmentation(self):
        ...
        if self._tiling_shape_layer_selection.value is not None:
            ...
            scale = [
                x for x in transforms if isinstance(x, Scale)
            ][0].scale
            # Rescale data;
            data_sd["geometry"] = (
                data_sd["geometry"]
            ...
            translate = [
                x for x in transforms if isinstance(x, Translation)
            ]
            if translate != []:
...
class TMAMeasurerNapariWidget(MultiScaleImageNapariWidget):
    ...
    def create_parameter_widgets(self):
        ...
        self._segmentation_layer_selection.changed.connect(
            self.update_model)
        ...
        self._extended_properties_toggle = create_widget(
            ...
            widget_type="CheckBox",
            options=dict(
                tooltip=f"Additional properties: {TMAMeasurer.EXTENDED_EXPORT_PROPERTIES}")
            )
        ...
        self._measure_labels_button = create_widget(
            ...
            widget_type="PushButton",
            options=dict(
                tooltip=f"Measure defaults properties: {TMAMeasurer.EXPORT_PROPERTIES}"
            )
        )
    ...
    def get_segmentation_layers(self, widget=None):
        return [
            l.name for l in self.viewer.layers if 
                isinstance(l, napari.layers.labels.labels.Labels) and
                "sdata" in l.metadata and
                "adata" in l.metadata
            ]
...
class TMAProcessorParentWidget(QTabWidget):
    ...
    def __init__(self, viewer: "napari.viewer.Viewer"):
        ...
        if init_selected is not None and "sdata" in init_selected.metadata:
            if isinstance(init_selected.data, napari.layers._multiscale_data.MultiScaleData):
        ...
        self.dearrayer = TMADearrayerNapariWidget(self.viewer, init_model_dearrayer)
        self.dearrayer.max_width = 475
        self.dearrayer.max_height = 400
        self.addTab(self.dearrayer.native, "Dearrayer")

        self.segmenter = TMASegmenterNapariWidget(self.viewer, init_model_segmenter)
        self.segmenter.max_width = 475
        self.segmenter.max_height = 700
        self.addTab(self.segmenter.native, "Segmenter")

        self.measurer = TMAMeasurerNapariWidget(self.viewer, init_model_measurer)
