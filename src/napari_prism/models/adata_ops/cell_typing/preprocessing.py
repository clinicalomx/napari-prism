...
def log1p(adata: AnnData, copy: bool = True, **kwargs) -> AnnData | None:
    ...
    if backend == "cpu":
        ...
        out = sc.pp.log1p(adata, copy=copy, **kwargs)
    else:
        import rapids_singlecell as rsc

        rsc.get.anndata_to_GPU(adata)
        out = rsc.pp.log1p(adata, copy=copy, **kwargs)
        if copy:
...
def scale(
    ...
    if backend == "cpu":
        ...
        out = sc.pp.scale(adata, copy=copy, **kwargs)
    else:
        import rapids_singlecell as rsc

        rsc.get.anndata_to_GPU(adata)
        out = rsc.pp.scale(adata, copy=copy, **kwargs)
        if copy:
            rsc.get.anndata_to_CPU(out)
        else:
            rsc.get.anndata_to_CPU(adata)
...
def neighbors(
    ...
    **kwargs,
    ...
    if backend == "cpu":
        ...
        out = sc.pp.neighbors(adata, copy=copy, **kwargs)
    else:
        import rapids_singlecell as rsc

        rsc.get.anndata_to_GPU(adata)
        out = rsc.pp.neighbors(adata, copy=copy, **kwargs)
        if copy:
            rsc.get.anndata_to_CPU(out)
        else:
            rsc.get.anndata_to_CPU(adata)
