...
def test_multichannel_image_projection(
    singlescale_3d_image_dataarray: Any,
    sdata_image_operations_stateless: Any
) -> None:
    CHANNELS = ["c1", "c2"]
    out_max = sdata_image_operations_stateless.get_multichannel_image_projection(
        singlescale_3d_image_dataarray,
        CHANNELS, 
        'max'
    )
    out_max_expected = np.array([
        [1, 1, 1],
        [1, 1, 1],
        [1, 1, 1]
    ])

    out_mean = sdata_image_operations_stateless.get_multichannel_image_projection(
        singlescale_3d_image_dataarray,
        CHANNELS,
        'mean'
    )
    ...
    out_sum = sdata_image_operations_stateless.get_multichannel_image_projection(
        singlescale_3d_image_dataarray,
        CHANNELS,
        'sum'
    )

    out_median = sdata_image_operations_stateless.get_multichannel_image_projection(
        singlescale_3d_image_dataarray,
        CHANNELS,
        'median'
    )
...
def test_add_image_disk(
    image: Any,
    sdata_image_operations_disk: Any) -> None:
